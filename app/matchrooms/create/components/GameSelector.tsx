...
export default function GameSelector({ selectedGame, onSelectGame, userProfile }: GameSelectorProps) {
    const allGames = [
        ...GAME_OPTIONS.map(g => ({ key: g.key, label: g.label })),
        ...SPORT_OPTIONS.map(s => ({ key: s.key, label: s.label })),
    ];

    // Filter to only show games the user has added to their profile
    const activeGames = allGames.filter(game => isGameActive(game.key, userProfile));
