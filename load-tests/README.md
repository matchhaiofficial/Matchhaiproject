...
> NEVER hit Easypaisa production endpoints. Use the QA Convex deployment
> (`https://striped-dog-623.convex.cloud` / `https://striped-dog-623.convex.site`)
...
```
POST https://striped-dog-623.convex.cloud/api/query
...
2. **Mint via Better Auth on `.site`.** The auth routes under
   `https://striped-dog-623.convex.site/api/auth/*` can issue a session for a
