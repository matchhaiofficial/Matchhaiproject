...
export function useStartDiditKyc() {
  ...
  return useCallback(
    async (role: KycRole): Promise<KycStartResult> => {
      ...
      const request = (async (): Promise<KycStartResult> => {
        try {
          if (!isValidAccountEmail(user?.email)) {
            captureAnalyticsEvent("kyc_session_start_failed", {
              kyc_role: role,
              failure_category: "account_email_missing",
            });
          ...
          captureAnalyticsEvent("kyc_session_started", {
            kyc_role: role,
          ...
          return {
            ok: false,
            message: getUserFacingErrorMessage(error, "Could not start identity verification."),
          };
        }
      })();
