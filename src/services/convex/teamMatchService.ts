...
type ServerResponse =
  { ok: true; message?: string; [key: string]: any } | { ok: false; message: string; [key: string]: any };
...
const computeChallengePricePerPlayer = async (input: {
  ...
  try {
    ...
    if (!zone) return 0;
    const baseRate =
      toPositiveNumber(input.zoneRatePrice) || getZoneRateForChallenge(zone, input.gameKey, input.maxPlayers);
...
const toDurationMinutes = (gameKey: string, seriesType?: string | null) => {
  ...
  if (game === "cs2" || game === "cs16" || game === "valorant")
    return series === "BO3" ? 180 : series === "BO5" ? 300 : series === "BO10" ? 600 : 60;
  if (game === "fc26" || game === "fc25")
    return series === "BO3" ? 60 : series === "BO5" ? 120 : series === "BO10" ? 180 : 30;
...
const getTeamData = async (teamId: string) => {
  const team = await convex.query(api.teams.getWithMembers, {
    teamId: teamId as Id<"teams">,
  });
  if (!team) throw new Error("Team not found");

  const memberUids = dedupe([team.captainUid, ...team.memberUids]);
...
const getDefaultLineup = (team: any, members: any[]) => {
  ...
  const mainMembers = members
    .filter(
      (member: any, index: number) =>
        (member.rosterRole || (index < mainRosterSize ? "main" : "substitute")) === "main",
    )
  ...
  return mainMembers.length > 0
    ? mainMembers
    : dedupe([team.captainUid, ...(team.memberUids || [])])
        .slice(0, mainRosterSize)
        .map(String);
};
...
export const sendTeamMatchChallenge = async (input: {
  ...
  const trackFailure = (message: string, game?: string): ServerResponse => {
    captureAnalyticsEvent("team_challenge_create_failed", {
      game: game?.toLowerCase(),
      format: input.format || "unknown",
      series_type: input.seriesType || "unknown",
      max_players: input.maxPlayers,
      payment_mode: TEAM_CHALLENGE_PAYMENTS_ENABLED ? "paid" : "free",
      failure_category: classifyAnalyticsFailure(message),
    });
    return { ok: false, message };
  };
  try {
    const verificationGate = await requireKycAccess();
    if (!verificationGate.ok) return trackFailure(verificationGate.message);

    const me = await getCurrentUserInfo();
    if (!me) return trackFailure("Not authenticated.");

    if (!input.scheduledDate || !input.scheduledTime) {
      return trackFailure("Select day/date/time before sending challenge.");
    }
    if (!isValidVenueChoice(input.proposedVenueByCaptainA)) {
      return trackFailure("Select preferred zone before sending challenge.");
    }
    const scheduledAt = parseScheduledDateTime(input.scheduledDate, input.scheduledTime);
    if (!scheduledAt) {
      return trackFailure("Invalid challenge date/time.");
    }
    const scheduleError = validateTeamChallengeScheduledAt(scheduledAt.getTime());
    if (scheduleError) {
      return trackFailure(scheduleError);
    }
    ...
    if (teamA.captainUid !== me.convexId) {
      return trackFailure("Only your team captain can send a challenge.", teamA.game);
    }
    if (teamA._id === teamB._id) {
      return trackFailure("Choose another team to challenge.", teamA.game);
    }
    if (String(teamA.game || "").toLowerCase() !== String(teamB.game || "").toLowerCase()) {
      return trackFailure("Both teams must be from the same game.", teamA.game);
    }
    if (!isTeamFilled(teamA, teamAMembers) || !isTeamFilled(teamB, teamBMembers)) {
      return trackFailure("Team challenge can only be sent when both teams are full.", teamA.game);
    }
    ...
    const activePlayers =
      Number(teamA.mainRosterSize || getTeamMainRosterSize(teamA.game)) +
      Number(teamB.mainRosterSize || getTeamMainRosterSize(teamB.game));
    const maxPlayers =
      normalizedGame === "cs2" || normalizedGame === "cs16" || normalizedGame === "valorant"
        ? 10
        : activePlayers || computedPlayers;
    ...
    if (computedPricePerPlayer <= 0) {
      return trackFailure("Unable to derive price per player from selected zone and series.", teamA.game);
    }
    ...
    captureAnalyticsEvent("team_challenge_created", {
      game: normalizedGame || "unknown",
      format: input.format || "unknown",
      series_type: input.seriesType || "unknown",
    ...
    if (rawMessage.includes("already have a pending challenge")) {
      return trackFailure("You already have a pending challenge for this team.");
    }
    Logger.error("teamMatchService", "sendTeamMatchChallenge failed", error);
    return trackFailure(rawMessage || "Failed to send challenge.");
  }
};
...
export const acceptTeamMatchChallenge = async (input: {
  ...
  const trackFailure = (message: string, game?: string): ServerResponse => {
    captureAnalyticsEvent("team_challenge_accept_failed", {
      game: game?.toLowerCase(),
      payment_mode: TEAM_CHALLENGE_PAYMENTS_ENABLED ? "paid" : "free",
      failure_category: classifyAnalyticsFailure(message),
    });
  ...
  try {
    ...
    if (!challenge) return trackFailure("Challenge not found.");

    if (challenge.status !== "pending") {
      const statusText = String(challenge.status || "resolved").replace(/_/g, " ");
      return trackFailure(`Challenge is already ${statusText}.`, challenge.gameKey);
    }
    ...
    captureAnalyticsEvent("team_challenge_accepted", {
      game: String(challenge.gameKey || "unknown").toLowerCase(),
      payment_mode: TEAM_CHALLENGE_PAYMENTS_ENABLED ? "paid" : "free",
      outcome: "success",
    });
    ...
    return trackFailure(getUserFacingErrorMessage(error, "Failed to accept challenge."));
  }
};
...
export const startTeamChallengeEasypaisaCheckout = async (input: {
  ...
  try {
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to start Easypaisa checkout."),
    };
  }
};

// Reject team match challenge
export const rejectTeamMatchChallenge = async (input: { challengeId: string }): Promise<ServerResponse> => {
  try {
    const me = await getCurrentUserInfo();
    if (!me) return { ok: false, message: "Not authenticated." };
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to reject challenge."),
    };
  }
};
...
export const suggestTeamMatchChallengeAlternativeZone = async (input: {
  ...
  try {
    ...
    if (
      ...
      return {
        ok: false,
        message: "Accept the challenge before suggesting an alternative zone.",
      };
    }
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to suggest alternative zone."),
    };
  }
};
...
export const respondToTeamMatchChallenge = async (input: {
  ...
  try {
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to respond to challenge."),
    };
  }
};
...
export const proposeTeamChallengeVenue = async (input: {
  ...
  try {
    ...
    if (
      ...
      return {
        ok: false,
        message: "Challenge is not in venue proposal state.",
      };
    }
    ...
    if (result?.confirmedVenue) {
      ...
      if (!latest) {
        return {
          ok: false,
          message: "Challenge not found after venue confirmation.",
        };
      }
      ...
      const isPaidChallenge =
        ...
        (String((latest as any).paymentMode || "") === "paid" || Number(latest.pricePerPlayer || 0) > 0);
      ...
      const lineupA =
        Array.isArray(latest.lineupA) && latest.lineupA.length > 0
          ? latest.lineupA.map(String)
          : getDefaultLineup(teamAData.team, teamAData.members);
      const lineupB =
        Array.isArray(latest.lineupB) && latest.lineupB.length > 0
      ...
      const activePlayers = [
        ...lineupA.map((uid) => ({
          uid,
          username:
            teamAData.usernameByUid[uid] ||
            (uid === String(latest.captainAUid) ? latest.captainAName : "Team A Player") ||
            "Team A Player",
          joinedAt: Date.now(),
          role: "Team A",
        })),
        ...lineupB.map((uid) => ({
          uid,
          username:
            teamBData.usernameByUid[uid] ||
            (uid === String(latest.captainBUid) ? latest.captainBName : "Team B Player") ||
            "Team B Player",
          joinedAt: Date.now(),
          role: "Team B",
        })),
      ...
      const matchroom = await createMatchroom({
        ...
        pricing: {
          perPlayer: Number(latest.pricePerPlayer || 0),
          currency: "PKR",
        },
      ...
      if (!matchroom.ok || !matchroom.id) {
        return {
          ok: false,
          message: matchroom.message || "Failed to create challenge matchroom.",
        };
      }
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to propose venue."),
    };
  }
};
...
export const getTeamMatchChallengeById = async (challengeId: string): Promise<ServerResponse> => {
  try {
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to load challenge."),
    };
  }
};
...
export const getCaptainedTeams = async (uid: string): Promise<ServerResponse> => {
  try {
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to load captain teams."),
    };
  }
};
...
export const getChallengesForCaptain = async (uid: string): Promise<ServerResponse> => {
  try {
    ...
    return {
      ok: false,
      message: getUserFacingErrorMessage(error, "Failed to load challenges."),
    };
  }
};
