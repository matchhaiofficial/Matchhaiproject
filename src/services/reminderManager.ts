...
export function buildUpcomingMatchReminderPlans(input: {
  ...
  const plans = (input.upcomingRooms || [])
    .map((room): ReminderPlan | null => {
      ...
      // Never schedule reminders for finished/abandoned rooms.
      const status = String(room?.status || "").toLowerCase();
      if (status === "completed" || status === "cancelled" || status === "expired") return null;
      const startAt = getRoomStartDate(room);
      if (!startAt) return null;
      const startAtMs = startAt.getTime();
      // Skip invalid timestamps and rooms that have already started — a past
      // start time must never produce an immediate "starts soon" reminder.
      if (!Number.isFinite(startAtMs) || startAtMs <= now) return null;
      const triggerAtMs = startAtMs - minutesBefore * 60 * 1000;
      // If the reminder moment is already in the past, don't schedule it.
      if (triggerAtMs <= now) return null;
