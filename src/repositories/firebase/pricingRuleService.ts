...
export async function getEnabledPricingRulesForZone(
    ...
    try {
        const q = query(
            collection(db, "zones", zoneId, "pricing_rules"),
            where("isEnabled", "==", true),
        );
        const snapshot = await getDocs(q);
        const rules = snapshot.docs.map((item: any) =>
            normalizeRule(zoneId, item.id, item.data() as Record<string, any>),
        );
...
export function subscribeZonePricingRules(
    ...
    const rulesRef = collection(db, "zones", zoneId, "pricing_rules");
    return onSnapshot(
        rulesRef,
        (snapshot: any) => {
        ...
        (error: any) => {
            if (error?.code !== "permission-denied") {
                Logger.error("pricingRuleService", "Rules listener failed", error);
            }
            onError(error);
        },
    );
...
export async function createZonePricingRule(
    ...
    try {
        await addDoc(collection(db, "zones", zoneId, "pricing_rules"), {
...
export async function setZonePricingRuleEnabled(
    ...
    try {
        await updateDoc(doc(db, "zones", zoneId, "pricing_rules", ruleId), {
            isEnabled,
            updatedAt: serverTimestamp(),
        });
        zoneRulesCache.delete(zoneId);
...
export async function deleteZonePricingRule(zoneId: string, ruleId: string) {
    try {
        await deleteDoc(doc(db, "zones", zoneId, "pricing_rules", ruleId));
        zoneRulesCache.delete(zoneId);
        return { ok: true as const };
    } catch (error: any) {
