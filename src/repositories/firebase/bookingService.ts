...
export async function getBookingIntentsByCreator(
    userUid: string
): Promise<{ ok: true; data: BookingIntent[] } | { ok: false; message: string }> {
    try {
        const q = query(
            collection(db, "booking_intents"),
        ...
        const data = snap.docs.map((docSnap) => ({ id: docSnap.id, ...docSnap.data() } as BookingIntent));
        return { ok: true, data };
    } catch (error) {
        Logger.error("bookingService", "Error fetching booking intents by creator", error);
